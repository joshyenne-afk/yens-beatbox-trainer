...
function renderPatternsOptimized(searchVal) {
    ...
    for (let idx = 0; idx < patterns.length; idx++) {
        ...
        if (gOk && sOk) {
            const active = seq.pattern && seq.pattern.name === p.name ? ' active' : '';
            html += `<div class="pattern-item${active}" onclick="loadPattern(patterns[${idx}])"><div class="pattern-item-name">${p.name}</div><div class="pattern-item-meta">${p.genre} · ${p.tempo} BPM</div></div>`;
        }
    }
