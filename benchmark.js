...
function renderPatternsOptimized(searchVal) {
    ...
    // We can also optimize the map step. Notice how indexOf is called in the loop! That's O(N^2)
    // To solve this, we can just use reduce
    const html = patterns.reduce((acc, p, idx) => {
        const gOk = activeGenre === 'All' || p.genre === activeGenre;

        // Cache lower case strings directly on the object. The objects are static and rarely change
        p._nameLower = p._nameLower || p.name.toLowerCase();
        p._descLower = p._descLower || p.desc.toLowerCase();
        p._genreLower = p._genreLower || p.genre.toLowerCase();
